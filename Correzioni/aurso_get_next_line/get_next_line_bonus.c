...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/09 18:27:37 by aurso             #+#    #+#             */
/*   Updated: 2022/11/09 18:27:40 by aurso            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || read(fd, 0, 0) < 0)
		return (NULL);
	fub[fd] = ft_get_buff(fub[fd], fd);
	print = ft_get_print(fub[fd]);
	fub[fd] = ft_get_fub(fub[fd]);
	return (print);
}
